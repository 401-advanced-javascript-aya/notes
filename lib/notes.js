...
class Notes {
  ...
  async execute() {
    ...
    if (this.data.delete) {
      // delete 1 note with --delete id
      let deletedNote = await noteDB.deleteOne(this.data.delete);
      console.log(`Deleted Note ${deletedNote}`);
    }

    if (this.data.methods[4]) {
      // delete bunch of notes by category or all if stated with --clear category or --clear
      let deletedNotes = await noteDB.deleteBunch(this.data.deleteFilter);
      console.log(
        `Deleted All Notes ${this.data.deleteFilter ? 'in Category ' + this.data.deleteFilter : ''
        }`,
        deletedNotes,
      );
    }
    ...
    if (this.data.updateID) {
      // updates a note with -u/--update with the selected ID, along with passed text, category parameters to change
      let updatedNote = await noteDB.updateOne(this.data);
      console.log(
        updatedNote
          ? `Updated Note ${updatedNote}`
          : `Note with ID:${this.data.updateID} non existent`,
      );
    }
  ...
  valid() {
    return (
      (this.data.methods && // checks if methods exists
